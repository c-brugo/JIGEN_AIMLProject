...
def get_train_dataloader(args, beta = 0, DA = False, self_sup_transformer=None, odd_one_out=False):
    ...
    #in case of domain adaptation, add the target domain datasets with no labels
    if DA:
        target_dataset_list = args.target
        if type(target_dataset_list) is not list:
        ...
        for target_dname in target_dataset_list:
            ...
            target_train_dataset = Dataset(name_train, labels_train, args.path_dataset, img_transformer=img_transformer, jig_transformer=self_sup_transformer, beta=beta, odd_one_out=odd_one_out)
