...
def get_args():
    parser = argparse.ArgumentParser(description="Script to launch odd_one_out training",
    ...
    parser.add_argument("--n_perm", "-p", type=int, default=9, help="Number of possible positions for the external element")
...
class Trainer:
    def __init__(self, args, device):
        ...
        self.jigsaw_editor = DatasetEdit(img_dim = args.image_size, P = args.n_perm)

        self.source_loader, self.val_loader = data_helper.get_train_dataloader(args, beta = args.beta, self_sup_transformer = self.jigsaw_editor.odd_one_out, DA = True, odd_one_out=True)
    ...
    def _do_epoch(self):
        ...
        for it, (data, class_l, perm_l) in enumerate(self.source_loader):
            ...
            if self.alpha != 0:
                losses_log["Odd-One-Out Source Loss "] = perm_source_loss.item()
                accuracy_log["Odd-One-Out Source Accuracy "] = torch.sum(perm_source_pred == perm_l_source.data).item()
            if self.alpha_t != 0:
                losses_log["Odd-One-Out Target Loss "] = perm_target_loss.item()
                accuracy_log["Odd-One-Out Target Accuracy "] = torch.sum(perm_target_pred == perm_l_target.data).item()
