...
class Trainer:
    def __init__(self, args, device):
        ...
        self.source_loader, self.val_loader = data_helper.get_train_dataloader(args, beta = args.beta, self_sup_transformer = self.jigsaw_editor.odd_one_out, odd_one_out=True)
        ...
        for it, (data, class_l, perm_l) in enumerate(self.source_loader):
            ...
            if self.alpha != 0:
                self.logger.log(it, len(self.source_loader),
                                ...
                                "Odd-One-Out Loss ": perm_loss.item(),
                                "Total Loss ": loss.item()},
                                {"Class Accuracy ": torch.sum(cls_pred == class_l.data).item(),
                                "Odd-One-Out Accuracy ": torch.sum(perm_pred == perm_l.data).item()},
                                data.shape[0])
